...
#        self.goalMsg.pose.orientation.w = 1.0
        ...
        if(self.goalId == 0):
            self.goalMsg.pose.orientation.z = 0.96
            self.goalMsg.pose.orientation.w = -0.25
    ...
    def statusCB(self, data):
        ...
        if data.status.status == 3: # reached
            ...
            if(self.goalId == 1):
                self.goalMsg.pose.orientation.z = 1
                self.goalMsg.pose.orientation.w = 0.008
...
if __name__ == "__main__":
    try:    
        ...
        # Get params
        goalListX = rospy.get_param('~goalListX', '[2.5, 3.1, 0.2]')
        goalListY = rospy.get_param('~goalListY', '[0.5, -0.50, -0.5]')
